...
/*
	...
	while(1)
	{
		...
		err(serverFd);
		response = readResponse(serverFd,&bytesRead);
		errPtr(response);
		printf("%s\n",response);
		ret = writeRespone(serverFd,response,&bytesRead);
		err(ret);
		//wait for the socket to be free from the last time
	}
